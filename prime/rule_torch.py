...
def _default(*a) -> Optional[Tag]:
    ...
    if isinstance(self_tag, TagSack):
        ...
        if not all(
            not isinstance(t, TagSack) for t in tags + list(kwtags.values())
        ):
    ...
    # TODO: release this constraint
        ...
        # TODO: handle iadd
        return Tag.merge(
            hash(method), self_tag + tags + kwtags, method == "__add__"
        )
    ...
    if isinstance(self_tag, TagSack):
        if method in ("mean", "std", "sum"):
            ...
            arg_is_safe = (
                ...
                or ((type(arg) in (list, tuple)) and arg[0] == 0)
            )

            tag = (
                SafeTag() if arg_is_safe and self_tag.is_safe() else DangerTag()
            )
...
def taint_torch(
    ...
    kwtags: Dict[str, Union[Tag, TagSack]],
) -> Tag:

    _rule = (
        rule_table[method.__name__]
        if method.__name__ in rule_table
        else _default
    )
