    ...
    if baseline:
        ...
        class PrimeDataset(TensorDataset):
            ...
            def __getitem__(self, index):
                tensors = tuple(
                    (self.transforms(t[index]) if i < self.n - 1 else t[index])
    ...
    if baseline:
        ...
        dlib = (
            ...
            if dataset == "cifar10"
                ...
                utkface
                if dataset == "utkface"
                else chestxray if dataset == "chestxray" else None
            )
        )
        assert dlib
    ...
    if dataset == "cifar10":
        ...
        train_transform = torchvision.transforms.Compose(
            [
                ...
                torchvision.transforms.ToTensor(),
                torchvision.transforms.Normalize(UTKFACE_MEAN, UTKFACE_STD),
            ]
        )
...
def test_image_classification(
    baseline: bool, dataset: str, model: str, max_epochs: str
):
    ...
    train_transforms, test_transforms = build_transforms(
        dataset_name, model_name, samples, labels
    )
    ...
    trainer = Trainer(accelerator="auto", devices="auto", max_epochs=max_epochs)
