...
def _connect_multi_assign_node(multi_assign_node, next_node):
    """Connect a MultiAssignNode properly to the next_node."""
    last = multi_assign_node.last
    if isinstance(next_node, MultiAssignNode):
        last.connect(next_node.first)
...
def connect_nodes(nodes):
    ...
    for n, next_node in zip(nodes, nodes[1:]):
        if isinstance(n, MultiAssignNode):
            _connect_multi_assign_node(n, next_node)
        elif isinstance(n, ControlFlowNode):
            _connect_control_flow_node(n, next_node)
        elif isinstance(next_node, MultiAssignNode):
            n.connect(next_node.first)
        elif isinstance(next_node, ControlFlowNode):
...
def get_last_statements(cfg_statements):
    ...
    if isinstance(cfg_statements[-1], MultiAssignNode):
        return [cfg_statements[-1].last]
