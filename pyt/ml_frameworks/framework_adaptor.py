""" A generic framework adaptor that leaves taint criteria to the caller. """
...
class FrameworkAdaptor():
    ...
    def __init__(
        ...
        self.is_training_entry = is_training_entry
        self.run()
    ...
    def find_training_entry_taint_args(self):
        """Find all training entry and taint all of their arguments.

        Yields:
            CFG of each entry function, with args marked as tainted.
        """
        for definition in _get_func_nodes():
            if self.is_training_entry(definition.node):
                yield self.get_func_cfg_with_tainted_args(definition)

    def run(self):
        """Run find_training_entry_taint_args on each CFG."""
        function_cfgs = list()
        for _ in self.cfg_list:
            function_cfgs.extend(self.find_training_entry_taint_args())
